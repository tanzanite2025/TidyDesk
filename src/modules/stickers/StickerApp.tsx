...
export const StickerApp: React.FC = () => {
  ...
  useEffect(() => {
    if (!stickerId) return;
    (nativeApi.isAvailable() ? nativeApi.stickers.get(stickerId) : Promise.resolve(null))
      .then(setSticker)
      .catch(() => setSticker(null));

    const unsubscribe = nativeApi.stickers.onUpdated(payload => {
  ...
  const togglePin = async () => {
    if (!sticker) return;
    const result = nativeApi.isAvailable() ? await nativeApi.stickers.togglePin(sticker.id) : undefined;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button type="button" title="复制图片" onClick={() => nativeApi.isAvailable() && nativeApi.stickers.copy(sticker.id)} className="rounded-md p-1.5 hover:bg-white/10">
          <Copy size={14} />
        </button>
        <button type="button" title="另存为" onClick={() => nativeApi.isAvailable() && nativeApi.stickers.saveAs(sticker.id)} className="rounded-md p-1.5 hover:bg-white/10">
          <Download size={14} />
        </button>
        <button type="button" title="关闭贴纸" onClick={() => nativeApi.isAvailable() && nativeApi.stickers.close(sticker.id)} className="rounded-md p-1.5 hover:bg-rose-500/20 hover:text-rose-100">
          <X size={14} />
        </button>
      </div>
    </div>
