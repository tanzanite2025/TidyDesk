...
import { QuickNotesPanel } from './modules/notes/QuickNotesPanel';
...
import { FileTile } from './modules/drawer/FileTile';
import {
  AlertTriangle,
  Check,
  ChevronLeft,
...
function getPathFromDroppedFile(file: globalThis.File): string | null {
  ...
  if (nativeApi.isAvailable() && !isTauriRuntime()) return nativeApi.windows.getPathForFile(file);
  return null;
}
...
const DrawerApp: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = nativeApi.windows.onDrawerState(payload => {
  ...
  async function handleDropOnDrawer(event: React.DragEvent, drawerId: string) {
    ...
    if (nativeApi.isAvailable()) {
      nativeApi.events.send('file-dropped');
    }
  ...
  async function openAppPicker(folderName: string) {
    try {
      ...
      await nativeApi.apps.openPicker({ targetFolder: folderName });
    } catch (err) {
...
const App: React.FC = () => {
  ...
  if (windowMode === 'handle') {
    ...
    content = <TodoPanelApp client={nativeApi} />;
